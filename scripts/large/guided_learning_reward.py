...
for e in range(epochs):
    ...
    for i in range(expert_trajectories.shape[0]):
        ...
        #target=target.to(device=args.device,dtype=torch.float32) 
    ...
    for exp_traj in range(numb_exp_trajectories):
        ...
        for step in range(expert_trajectories.shape[1]-step_size):
            ...
            if step % step_size==0:
                ...
                observation=expert_trajectories[exp_traj,step,2:].detach().cpu()
                ...
                #target=target.to(device=args.device,dtype=torch.float32) # to match prediction 
    ...
    if e==100:
        ...
        plt.ylabel('MSE Loss',fontsize=12)
        print(loss_array)
        plt.savefig(args.logbase+'/'+args.dataset+'/'+args.value_loadpath+'/loss_function_MSE_100.pdf',format="pdf", bbox_inches="tight")
