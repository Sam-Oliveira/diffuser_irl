...
def train(args,config=None):
    ...
    if config.train_normalizer=='medium-replay':
        choice_of_normalizer=diffusion_experiment.dataset.normalizer
    elif config.train_normalizer=='expert':
        choice_of_normalizer=dataset.normalizer
    else:
    ...
    policy_config = utils.Config(
        ...
        normalizer=choice_of_normalizer,
        preprocess_fns=args.preprocess_fns,
    ...
    for e in range(1,epochs+1):
        ...
        # need to re-do because of change in test function
        if config.train_normalizer=='medium-replay':
            policy.normalizer=diffusion_experiment.dataset.normalizer
        elif config.train_normalizer=='expert':
            policy.normalizer=dataset.normalizer
        else:
            raise Exception('Invalid normalizer')
...
def test(value_path,policy,diffusion_experiment,dataset,config=None,final=False,epoch=0):
    ...
    ## policies are wrappers around an unconditional diffusion model and a value guide
    if config.test_normalizer=='medium-replay':
        policy.normalizer=diffusion_experiment.dataset.normalizer
    elif config.test_normalizer=='expert':
        policy.normalizer=dataset.normalizer
    else:
        raise Exception('Invalid normalizer')
    ...
    for t in range(max_steps):
        ...
        action, samples = policy(conditions, batch_size=num_envs,diff_conditions=True,verbose=args.verbose)
...
def run_hyperparameter_sweep():
    ...
    parameters_dict={
        ...
        'lr':{
            ...
            'min':1e-4,
            'max':1e-1, #too large, need to change
        },
        'batch_size': {
