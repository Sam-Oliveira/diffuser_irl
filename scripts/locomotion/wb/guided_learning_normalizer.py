...
def train(args,config=None):
   ...
   # print(np.mean(dataset.fields.observations))
    ...
    for e in range(1,epochs+1):
        ...
        # eval
        if e==1:
            initial_reward(diffusion_experiment,dataset,config)
        elif (e-1)%10==0:
            value_function.eval()
        ...
        for i,targets in enumerate(train_dataloader):
            ...
            # only use first 50 batches for training, but they are shuffled at each epoch
            if i>50:
                break
...
def test(value_path,policy,diffusion_experiment,dataset,config=None,final=False,epoch=0):
    ...
    if final:
        wandb.log({'final_reward': 100*envs.envs[0].get_normalized_score(mean_reward)})
    else:
        wandb.log({'reward': 100*envs.envs[0].get_normalized_score(mean_reward)})

def initial_reward(diffusion_experiment,dataset,config=None):
    env=dataset.env
    num_envs=200

    # Initialize policy for unguided sampling
    policy_unguided = Policy(diffusion_experiment.ema, diffusion_experiment.dataset.normalizer)
    ...
    for t in range(max_steps):
        ...
        #need to normalize obs from env
        conditioning_obs=policy_unguided.normalizer.normalize(envs.observations, 'observations')
